"""HTTP adapter of the documents context: the write side's API.
...
provider functions from the composition root (`container.py`) — that is the
...
from knowledge_assistant.documents.infrastructure.http.mappers import (
    document_to_response,
    summary_to_response,
)
from knowledge_assistant.documents.infrastructure.http.schemas import (
...
async def ingest_document(
    ...
    app_container: Annotated[Container, Depends(container.get_container)],
    response: Response,
...
async def list_documents(
    use_case: Annotated[ListDocuments, Depends(container.provide_list_documents)],
...
async def get_document(
    ...
    use_case: Annotated[GetDocument, Depends(container.provide_get_document)],
) -> DocumentResponse:

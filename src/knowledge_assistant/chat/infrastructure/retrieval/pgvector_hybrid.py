...
class PgVectorHybridRetriever:
    """Implements the `ChunkRetriever` port against PostgreSQL + pgvector."""
    ...
    async def retrieve(self, question: str, limit: int) -> list[RetrievedChunk]:
        try:
            ...
            # contract; re-wrap it as the CHAT context's own signal so this
            # context exposes only its own domain errors (context decoupling).
            msg = (
                "The knowledge base is temporarily unavailable "
                "(embedding provider unreachable after retries). Please try again shortly."
            )
        ...
        try:
            ...
            msg = (
                "The knowledge base is temporarily unavailable "
                "(database unreachable). Please try again shortly."
            )
            raise RetrievalUnavailableError(msg) from exc
        return [
            RetrievedChunk(
